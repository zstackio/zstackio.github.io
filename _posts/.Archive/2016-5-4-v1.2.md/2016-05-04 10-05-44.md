...
Distributed EIP is added as a new service type of flat network provider in this version. Users can apply an EIP(normally a public IP) to a VM on a private network dynamically, without using a virtual router, since the EIP is created on the host where the VM is running. The D
